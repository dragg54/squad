...
const GoalCategory = ({goal}) => {
    ...
    return (
        <div onClick={(e) => e.stopPropagation()} className='w-[400px] mx-auto h-[600px] p-6 bg-white border rounded-md shadow-gray-300'>
            <h1 className="mt-3 font-extrabold text-xl pb-2 w-full border-b border-gray-300 inline-flex justify-between items-center"><span>Goal Categories </span><Button
            onClick={()=>{
                dispatch(closeModal())
            ...
            <div className="flex flex-col items-start mt-4">
                {
                    userGoalCategories && userGoalCategories.data && userGoalCategories.data.map(category =>(
                        <label key={category.id}>
                         <Input type="checkbox" style='ml-3'/>
                         {category.name}
                        </label>
                    ))
                }
