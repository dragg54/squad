    ...
    return (
        <div onClick={(e) => e.stopPropagation()} className='w-[350px] md:w-[400px] mx-auto h-[600px] p-6 py-3 -mt-3 bg-white border rounded-md shadow-gray-300'>
            ...
            <Button
                onClick={()=>{
                dispatch(closeModal())
                dispatch(openModal({component: <EditGoal {...{goal, localSelectedCategory, setIsUpdated}}/>}))
            }}
            ...
            <div className="flex flex-col items-start mt-4">
                {
                    userGoalCategories && userGoalCategories.data && userGoalCategories.data.map(category =>(
                        <label key={category.id}>
                         <Input name="" value={category.id} 
                         onChange={()=>handleSelectGoalCategory(category)} 
                         checked={localSelectedCategory?.name == category.name}
                          type="checkbox" style='ml-3'/>
                         {category.name}
                        </label>
                    ))
                }
